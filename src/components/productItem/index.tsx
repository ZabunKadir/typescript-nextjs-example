...
const ProductItem = ({
  ...
  return (
    <Link
      href={buttonHref}
      className={`flex flex-col w-full min-h-[400px] px-4 pt-6 py-12 space-y-6 rounded-md justify-between items-center border bg-gray-50 border-primary-mid hover:shadow-md hover:shadow-primary-light hover:bg-white duration-300 group ${className}`}
    >
        ...
        {text && (
          <div className="flex flex-col w-full justify-start items-start">
            {text.map((element, index) => (
              <span
                key={index}
                className="items-center w-full text-title-small max-w-[320px] h-full max-h-[100px] line-clamp-3 break-words"
              >
                <FontAwesomeIcon
                  fontSize={"10px"}
